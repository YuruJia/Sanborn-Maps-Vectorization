...
vector_folder = os.fsencode('data/modified')
...
for sheets in loop: #iterate over every mapsheet
    print("Start with Conversion of Map Sheet " + str(sheets) + ".")
    vector_path = 'data/modified/' + str(sheets) +'.shp'
    ...
    in_memory_path = 'data/' + sheets + '_modified.tif'
    ...
    gdal.RasterizeLayer(chn_ras_ds, [1], layer, options=['ATTRIBUTE=type_bui_4'])
    band = chn_ras_ds.GetRasterBand(1)
         
    brick_array = band.ReadAsArray()
    frame_array = brick_array.copy()
    print("The mask for brick buildings is being computed.") 
    print("Please wait...")    
    for cell in np.nditer(brick_array, op_flags=['readwrite']):
        if cell[...] == 1:
            ...
            cell[...] = 0
    print("The mask for framework buildings is being computed.") 
    print("Please wait...")          
    ...
    brick_out_path = 'data/'+str(sheets)+'_brick_modified.tif'
    frame_out_path = 'data/'+str(sheets)+'_frame_modified.tif'
    
    print("Finishing sheet "+str(sheets)+".")
