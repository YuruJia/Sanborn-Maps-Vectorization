...
brick_file = genfromtxt('brick_thresh.csv', delimiter=',') #load the threshold values from the csv files
...
for sheets in loop: #iterate over every mapsheet
    ...
    # we use a raster that resides in the computer memory, no need to write it out to disk here
        ...
        if [R, G, B] >= brick_min and [R, G, B] <= brick_max:
            building_type = 1
        if [R, G, B] >= frame_min and [R, G, B] <= frame_max:
            building_type = 2
...
input_file = csv.reader(open('6751.csv', 'r'))
r = []; g = []; b = []
...
building_type = 2
with fiona.open(vector_path) as buildings:
    buildings_schema = copy.deepcopy(buildings.schema)
    print(buildings_schema)
    with fiona.open(modified_path, 'w', buildings.driver, buildings.schema, buildings.crs) as mod_building:
        for house in mod_building:
            mod_building.write({"properties": building_type})
