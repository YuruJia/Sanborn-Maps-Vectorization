    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for map, label in train_loader:
                ...
                assert map.shape[1] == net.n_channels, \
                ...
                with torch.cuda.amp.autocast(enabled=amp):
                    ...
                    loss = criterion(masks_pred, label) \
                           + dice_loss(F.softmax(masks_pred, dim=1).float(),
                                       F.one_hot(label, net.n_classes).permute(0, 3, 1, 2).float(),
                                       multiclass=False)
        ...
        if save_checkpoint:
            ...
            torch.save(net.state_dict(), str(dir_checkpoint / 'checkpoint_batchsize_{}_epoch{}.pth'.format(batch_size,epoch + 1)))
            logging.info(f'Checkpoint_bathsize_{batch_size}_ {epoch + 1} saved!')
